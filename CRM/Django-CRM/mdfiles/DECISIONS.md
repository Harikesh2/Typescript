...
| D-25 | 2026-08-13 | DB schema healthcheck: `website/checks.py` system check (Warning level) + `python manage.py healthcheck` command + compose `web` healthcheck and command-chain gate (`migrate && healthcheck && gunicorn`) | Detect-and-fail guarantees the API never serves traffic on a broken schema; `migrate` stays the auto-creator. Warning level (not Critical) so a fresh DB can still run `migrate` — Django runs system checks before applying migrations, so Critical would deadlock first-time setup. On Django 6.1 the check must be imported explicitly via `website/apps.py` `ready()` (app `checks.py` is no longer auto-loaded) and registered untagged (`@checks.register`), because `runserver` skips `Tags.database`-tagged checks when no DB alias is passed | Active |
